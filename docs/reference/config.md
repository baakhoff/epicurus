...
| `portability_max_file_mb` | `PORTABILITY_MAX_FILE_MB` | `int` | `512` | Per-file ceiling for the file-space half of an export — a file is read whole to be added to the archive. A file above it is omitted and **named** in the job's `reason`. `0` removes the limit. |
