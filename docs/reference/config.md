...
| `agent_max_steps` | `AGENT_MAX_STEPS` | `int` | `4` | **Default** max tool-calling rounds per agent turn; the operator can override it at runtime (`llm_prefs.agent_max_steps`, set via the Models/Settings UI — #297) without a restart. **No ceiling** since #925 (ADR-0143) — set it as high as the work needs, at the cost of time and tokens; the pref is floored at 1 and what bounds a turn is the loop's behaviour guards (repeated-call stop, tool-error streak, the deadline below), not this number. |
| `agent_turn_deadline_s` | `AGENT_TURN_DEADLINE_S` | `int` | `0` | Per-turn wall-clock budget in **seconds**; `0` disables it. The last backstop behind the behaviour guards (#925, ADR-0143), checked *between* tool rounds and never mid-call — a single hung tool is already bounded by the MCP read timeout. When it fires the turn still delivers an answer, the final round is told the time ran out so the reply says so, and `AgentTurn.stopped` is `"deadline"`. Off by default because a deadline can kill exactly the long turn the lifted round bound exists to allow. |
...
| `playbook_reflection_enabled` | `PLAYBOOK_REFLECTION_ENABLED` | `bool` | `true` | The reflection pass's **own off-switch** (#762) — before it, silencing reflection meant disabling *all* nightly maintenance. Default on now that the pass is **playbook-only** (it can no longer propose base-instructions edits — see [core-app § Governed playbooks](../services/core-app.md#governed-playbooks-adr-0093)). When `false` the nightly job reports `skipped — disabled` without spending a gateway call. |
...
| `files_scan_fuse_enabled` | `FILES_SCAN_FUSE_ENABLED` | `bool` | `true` | **Mass de-index fuse** (#848): refuse a file-space scan's purge when the rows it would delete look wholesale rather than editorial — a stale or empty mount must not reconcile `core_files` to zero. Set `false` to restore the pre-#848 behaviour. See [file space](files.md#configuration-core-app). |
| `files_scan_fuse_max_delete_ratio` | `FILES_SCAN_FUSE_MAX_DELETE_RATIO` | `float` | `0.5` | Share of a namespace's indexed rows whose purge in one scan is treated as suspect. |
| `files_scan_fuse_min_deletions` | `FILES_SCAN_FUSE_MIN_DELETIONS` | `int` | `5` | Absolute floor for the ratio rule; a store that lists nothing at all while rows exist trips at any size. |
| `files_external_mounts` | `FILES_EXTERNAL_MOUNTS` | `str` | `""` | Operator-declared drive-style mounts (#731): comma-separated `name:container-path[:ro\|rw]` entries, mode defaults to `ro`. A malformed entry, a duplicate name, or an invalid mode fails startup. See [file space → External mounts](files.md#external-mounts-731). |
| `files_external_mounts_indexed` | `FILES_EXTERNAL_MOUNTS_INDEXED` | `str` | `""` | Comma-separated mount *names* (from `files_external_mounts`) opted into indexing/watching. A mount not listed here is browsable/read/writable but never scanned or searched — the safe default for a whole-drive mount. |
| `files_external_mounts_exclude` | `FILES_EXTERNAL_MOUNTS_EXCLUDE` | `str` | `""` | Exclude globs for indexed mounts, scoped per mount name: `name=pat1\|pat2;name2=pat3`. Ignored for a mount not in `files_external_mounts_indexed`. |
| `portability_staging_dir` | `PORTABILITY_STAGING_DIR` | `Path` | `/tmp/epicurus-portability` | Where a tenant export is assembled and an uploaded archive is staged (#867). A **disposable cache** (constraint #2): the durable state is the job row that points here, so a restart may empty it and the download route answers `410` rather than pretending. Never a named volume, and never the host home dir. |
| `portability_max_file_mb` | `PORTABILITY_MAX_FILE_MB` | `int` | `512` | Per-object ceiling for the byte halves of an export: each file in the file space (read whole to be added to the archive) **and** each blob a module carries (#876, streamed through staging, never held whole). Anything above it is omitted and **named** in the component's `reason`; a blob's catalogue record still travels, and the import reports it under `missing`. `0` removes the limit. |
| `portability_max_archive_mb` | `PORTABILITY_MAX_ARCHIVE_MB` | `int` | `4096` | Ceiling on an uploaded archive; over it the upload is refused with `413` rather than filling the staging disk. `0` removes the limit. |
| `portability_retention_hours` | `PORTABILITY_RETENTION_HOURS` | `int` | `24` | How long a staged export/import survives before the next job sweeps it (and its row). |
...
| `KNOWLEDGE_INDEX_FUSE_ENABLED` / `KNOWLEDGE_INDEX_FUSE_MAX_DELETE_RATIO` / `KNOWLEDGE_INDEX_FUSE_MIN_DELETIONS` | `true` / `0.5` / `5` | knowledge | The knowledge-side **mass de-index fuse** (#848) — the same guard as `FILES_SCAN_FUSE_*`, over the note/doc ledgers and their Qdrant collections: an index pass whose deletions look wholesale is refused rather than reconciling a stale mount into a wiped knowledge base. `force=true` on `knowledge_reindex` / `POST /reindex` overrides one pass. See [knowledge](../services/knowledge.md#the-mass-de-index-fuse-848). |
