...
| `mail.received` | `mail` | `{message_id, from, subject (≤200 chars), folder, has_attachments: bool, provider}` | the provider message id | The first provider-backed emitter ([mail](../services/mail.md)). Fired from the cache reconcile (ADR-0096, #623) once per genuinely-new message. Reconcile runs on a mailbox page read **and** on mail's [background poller](../services/mail.md#background-reconcile-796) (`MAIL_POLL_INTERVAL_S`, default 300s), so since #796 this event does **not** depend on anyone having the Mail page open — that is what lets an automation or a push alert on new mail run unattended. **Never** fired on a *first-ever* sync (no-firehose); a sync that *resumes* a mailbox synced before replays what arrived during the gap, capped at 50 messages. Carries an `EntityRef` of kind `message`. See *Provider caveats* below. |
...
| `calendar.event_created` | `calendar` | `{title (≤200 chars), start, end, all_day: bool}` | `"<provider>:<event_id>"` | **Two emitters, never both for one change** ([calendar](../services/calendar.md)): `CollectionRouter.create_event` — the provider-write seam, immediate — and the [reconcile loop](../services/calendar.md#reconcile-layer--external-changes-reach-the-spine-831) (#831, `SYNC_POLL_INTERVAL_S`, default 300s) for an event created *outside* epicurus, which is what lets an automation on "a calendar change" run for changes the operator made in Google's own UI. A write through the module records a short-lived self-write marker the reconcile consumes instead of re-announcing. **Never** fired on a first-ever sync of a collection (no-firehose); a whole recurring series is one event keyed on the series, not one per occurrence. Carries an `EntityRef` of kind `event`. See *Provider caveats* below. |
| `calendar.event_updated` | `calendar` | `{title, start, end, all_day, time_changed: bool}` | `"<provider>:<event_id>:<change hash>"` | Fired by `CollectionRouter.update_event` **or** the reconcile loop (#831), under the same suppression. `time_changed` is a real before/after comparison — the router against the pre-write state it fetches first, the reconcile against its cached observation — not inferred from which args were passed; only where the router cannot resolve the prior state does it degrade to "were `start`/`end` actually supplied." The change hash means a genuinely different edit is its own log entry; an identical retried write dedups. |
| `calendar.event_cancelled` | `calendar` | `{title (≤200 chars)}` | `"<provider>:<event_id>"` | Fired by `CollectionRouter.delete_event` after the delete actually succeeds (the event's prior state is snapshotted first — nothing is left to read once it's gone), **or** by the reconcile loop (#831) on a provider tombstone, where the title comes from the reconcile's own cache. A tombstone for an event this module never observed emits nothing — it never said that event existed. |
| `calendar.event_starting_soon` | `calendar` | `{title, start, end, lead_minutes: int}` | `"<provider>:<event_id>:starting_soon"` | Fired by the lead-time scheduler (`epicurus_calendar.scheduler`, the first of calendar's two periodic background jobs) when an event enters its lead window (tenant setting, default 15 minutes). Fire-once via a durable marker — proven to survive a restart. |
