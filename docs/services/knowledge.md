...
"change that suggestion" revises it in place instead of piling up a near-duplicate. Update
and withdraw stay inside the pending queue and never touch the vault; approve/reject stay
off the MCP surface entirely (exposing them would let the agent approve its own proposals):

| Tool | Inputs | Returns |
| --- | --- | --- |
| `knowledge_list_suggestions(status="pending")` | `status`: must be `"pending"` (the only queryable state) | The pending queue: id, kind, target, proposed-at, and a short content preview, one per line. **Check this before proposing** — revise a similar pending suggestion instead of duplicating it. |
| `knowledge_read_suggestion(id)` | `id`: a suggestion id | The full proposed content and target for one pending suggestion. |
| `knowledge_update_suggestion(id, content=None, note=None, to_path=None)` | `id`: a suggestion id; the rest: whichever fields to revise (omit to leave unchanged) | A confirmation that the suggestion was revised in place — still pending, `proposed-at` refreshed, one queue entry (never two). |
| `knowledge_withdraw_suggestion(id)` | `id`: a suggestion id | A confirmation that the suggestion was withdrawn (removed from the pending queue, kept as history). |

`knowledge_update_suggestion` and `knowledge_withdraw_suggestion` refuse (raised error, MCP
...
**A `delete` whose target is already gone is not a silent success (#761).** If the vault
document was already removed by the time the suggestion is approved (deleted some other way,
or a duplicate suggestion resolved first), the file API 404s. Unlike every other failure
`approve` can hit, this one does **not** leave the suggestion pending: index cleanup still
runs and the row is still resolved (audited, dropped from the queue) exactly as a real delete
would be — a stale suggestion must not be left un-resolvable just because there is nothing
left to remove. But the caller is told the truth instead of a false "approved": `approve`
raises a 404 naming the path and suggesting a next step (e.g. `"projects/plan.md" does not
exist — it may have been deleted or moved; list the folder for current contents`), so the
Suggestions UI shows an error (not a silent close) and, with review off, the auto-apply tool
call fails structurally instead of printing "applied directly" for a no-op.
...
- `POST /pages/review/suggestions/{id}/approve` — apply the change and drop it from the
  ...
  agent's full proposal unedited. **404** if a `delete`'s target no longer exists (#761,
  see above) — the suggestion is resolved either way, this only shapes the response.
...
**A width the guard cannot read is never cached (#944, ADR-0141).** `config.params.vectors` is
one unnamed `VectorParams`, a mapping of named ones, or absent. A single-entry mapping is still
one unambiguous width and is read as such; several named vectors have no single width to
compare, so the guard leaves the collection alone rather than guessing one and dropping data on
the guess — but it no longer *caches* the width it merely asked for. Caching an unconfirmed
width claims a check that never happened, so every later pass would skip it for the process's
lifetime while every upsert kept failing. That exact defect, in the core's twin of this guard,
is what made #944's recall failure repeat identically for the life of the process.

**Search names the change; it never rebuilds (#879).** Search is where a switched embedding
model is noticed first — a query embedded at the new width is rejected by the same opaque
`Vector dimension error`, which reads like a backend fault. `KnowledgeIndexer.search` therefore
maps it: on a rejected query it re-reads the collection's configured width (error path only, so
the happy path pays nothing) and, if it really no longer matches, raises
`dimensions.EmbeddingDimensionMismatch` — "the embedding model changed — this index holds 768-d
vectors and the current model produces 4096-d … run “Re-embed everything” on the Models page".
That sentence is what the agent sees as the tool's error (ADR-0136), so the model can relay a
cause the operator can act on. Rebuilding stays the indexer's job: a rebuild triggered by a
*read* would silently drop every vector the operator is still searching. Any other backend
failure is forwarded unchanged — Qdrant being down stays Qdrant being down.
...
| `dimensions.py` | The vector-dimension guard (#865): `CollectionDimensionGuard` (one per Qdrant collection — confirms the width, recreates on a change, and clears every registered ledger; shared by the two sources on `<tenant>__docs`), `EmbeddingDimensionChanged`, the restart signal the indexers catch to re-walk from scratch, and — for the read side (#879, ADR-0141) — `vector_size` (the one place a collection's configured width is read) plus `named_mismatch` / `EmbeddingDimensionMismatch`, which turn a rejected *query* into the cause and the cure without rebuilding anything. |
...
| `suggestions.py` | The `review` page surface (#220, ADR-0033; edit-before-approve + audit, ADR-0090): the `knowledge_suggestions` store (with the added `to_path` column; `SuggestionStore.update` revises a pending row in place, refreshing `created_at` — #744), `SuggestionReview` (diff + apply on approve / discard on reject, across create/update/delete/move/mkdir/mkproject; approve takes optional edited `content`; `read`/`update`/`withdraw` — #744 — are the agent-facing pending-only lifecycle ops, 404ing via `_unresolved_reason` with the resolved verdict when a suggestion is no longer pending), the `knowledge_suggestion_decisions` audit store (`SuggestionAuditStore`, capped at `MAX_DECISIONS`; `.get` looks up one decision by id — #744), and `create_review_router`. The shared wire contract (`ReviewSuggestion`/`ReviewData`/`ApplyResult`/`ApproveBody`/`ReviewDecision`/`ReviewAuditData`) is imported from `epicurus_core.review`, not locally redefined. The review diff reads the current content through a `VaultReader` (#346/ADR-0070); apply writes through the core file API like the editor save (#356/ADR-0064). Approve/reject are operator-only — never MCP tools; `read_only` (watch mode, #232) 409s approve. A `delete` whose target already 404s does not silently "succeed" (#761): `approve` still resolves the suggestion (index cleanup, audit, dequeue) but then raises `SuggestionTargetGone` (an `HTTPException` subclass, 404) naming the path — the one case where `approve` raises *after* mutating state instead of before. |
