...
Alembic requires an ``env.py`` inside every script location, and core-app needs its own script
location because it owns its own ``versions/`` sequence. It does not need its own opinion about
how migrations are configured, so the whole body lives in
