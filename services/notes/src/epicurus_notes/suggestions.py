...
class NoteSuggestionStore:
    ...
    async def update(
        ...
        *,
        ...
        ``None`` for a field leaves it unchanged (``slug``/``operation``/``origin`` never
        change — a suggestion's identity and kind don't, only its content). ``created_at``
        refreshes to now. Returns the updated row, or ``None`` if *sid* isn't in the pending
        queue — unknown, or already resolved (approve/reject/withdraw all *delete* the row);
        the caller tells those apart via the audit trail.
        """
        async with self._session() as session:
            row = await session.scalar(
                select(_StoredNoteSuggestion).where(
...
def _decision_to_value(row: _StoredNoteDecision) -> ReviewDecision:
...
class NoteSuggestionAuditStore:
    ...
    async def get(self, *, tenant: str, sid: str) -> ReviewDecision | None:
        ...
        model-actionable message (#697 precedent) rather than a bare 404.
        """
        async with self._session() as session:
            row = await session.scalar(
                select(_StoredNoteDecision)
                .where(_StoredNoteDecision.tenant == tenant, _StoredNoteDecision.sid == sid)
                .order_by(_StoredNoteDecision.id.desc())
            )
...
class NoteSuggestionReview:
    ...
    async def read(self, sid: str) -> NoteSuggestion:
    ...
    async def update(
        self, sid: str, *, content: str | None = None, note: str | None = None
    ) -> NoteSuggestion:
        ...
        touches the note (the review gate is unchanged). 404 if unknown or already resolved
        — see :meth:`_unresolved_reason`.
        """
        updated = await self._store.update(
            tenant=self._tenant, sid=sid, proposed_content=content, note=note
        )
        if updated is None:
            raise HTTPException(status_code=404, detail=await self._unresolved_reason(sid))
        log.info("note suggestion revised", sid=sid, operation=updated.operation, slug=updated.slug)
        return updated
    ...
    async def withdraw(self, sid: str) -> ApplyResult:
        ...
        stale, not an operator's review verdict. 404 if unknown or already resolved — see
        :meth:`_unresolved_reason`.
        """
        s = await self._store.get(tenant=self._tenant, sid=sid)
        if s is None:
            raise HTTPException(status_code=404, detail=await self._unresolved_reason(sid))
        ...
        await self._store.delete(tenant=self._tenant, sid=sid)
        log.info("note suggestion withdrawn", sid=sid, operation=s.operation, slug=s.slug)
        return ApplyResult(id=sid, status="withdrawn", path=s.slug, operation=s.operation)
